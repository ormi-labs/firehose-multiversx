...
var StartCmd = &cobra.Command{Use: "start", Short: "Starts `firemultiversx` services all at once", RunE: firehoseCmdStartE, Args: cobra.ArbitraryArgs}
...
func firehoseCmdStartE(cmd *cobra.Command, args []string) (err error) {
	...
	rootLog.Debug("firemultiversx binary started", zap.String("data_dir", dataDir))

	configFile := viper.GetString("global-config-file")
	rootLog.Info(fmt.Sprintf("starting Firehose on Multiversx with config file '%s'", configFile))
